...
export default class Desktop {
    ...
    constructor({
        ...
        eventsBus.on('beforeLoadUrl', async () => {
            try {
                // Log absolute app paths for debugging
                const appPath = app.getAppPath();
                this.writeToLogFile(`Starting server processes from app path: ${appPath}`);
                this.writeToLogFile(`Electron app resources path: ${process.resourcesPath}`);
    ...
    */
    ...
    async startMongoDB(dbPath) {
        ...
        const possiblePaths = [
            ...
            path.join(process.resourcesPath, 'mongodb', mongodName),
            path.join(process.resourcesPath, 'app.asar.unpacked', 'mongodb', mongodName),
            ...
            path.join(process.resourcesPath, 'mongodb-binaries', process.platform, mongodName),
            // Add more potential paths for macOS specifically
            path.join(app.getAppPath(), '..', 'mongodb-binaries', process.platform, mongodName),
            path.join(process.resourcesPath, '..', 'mongodb-binaries', process.platform, mongodName)
        ];
        ...
        if (!mongodPath || !fs.existsSync(mongodPath)) {
            const errorMsg = `MongoDB binary not found. Checked paths: ${possiblePaths.join(', ')}`;
            this.writeToLogFile(errorMsg);
            
            // Display a more detailed error dialog with the paths that were checked
            this.displayPathErrorDialog(
                'MongoDB Binary Not Found',
                'Could not locate the MongoDB binary needed to run the application.',
                errorMsg,
                {
                    'Checked Locations': possiblePaths.join('\n')
                }
            );
    ...
    async startMeteorServer(mongoPort) {
        ...
        const possiblePaths = [
            ...
            path.join(process.resourcesPath, 'meteor-bundle'),
            path.join(process.resourcesPath, 'app.asar.unpacked', 'meteor-bundle'),
        ...
        if (!meteorBundlePath) {
            const errorMsg = `Meteor bundle not found. Checked paths: ${possiblePaths.join(', ')}`;
            this.writeToLogFile(errorMsg);
            
            // Display a more detailed error dialog
            this.displayPathErrorDialog(
                'Meteor Bundle Not Found',
                'Could not locate the Meteor application bundle needed to run the application.',
                errorMsg,
                {
                    'Checked Locations': possiblePaths.join('\n')
                }
        ...
        if (!fs.existsSync(mainJsPath)) {
            const errorMsg = `Meteor bundle main.js not found at: ${mainJsPath}`;
            this.writeToLogFile(errorMsg);
            
            // Display a more detailed error dialog
            this.displayPathErrorDialog(
                'Meteor Application Not Found',
                'Could not locate the Meteor application entry point (main.js).',
                errorMsg,
                {
                    'Bundle Location': meteorBundlePath,
                    'Expected main.js': mainJsPath
                }
            );
        ...
        const env = {
            ...
            // Extra environment variables to help with debugging
            VERBOSE: '1',
            DEBUG: '1',
            METEOR_PRINT_CONSTRAINT_SOLVER_INPUT: '1',
            METEOR_DEBUG_CONSTRAINTS: '1'
        };
